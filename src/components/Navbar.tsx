...
export const Navbar = () => {
    ...
    return (
        <>
        <header className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4 md:px-5 transition-all duration-300 pointer-events-none">
                    ...
                    <div className="hidden lg:flex items-center gap-8">
                        {navItems.map(({ labelKey, id, href }) => (
                            <button
                                ...
                                className="text-[15px] font-bold text-slate-700 hover:text-slate-900 transition-colors bg-transparent border-none cursor-pointer"
                            >
                                {t(labelKey)}
                            </button>
                        ))}
                        ...
                        <button onClick={() => setModalOpen(true)} className="flex items-center gap-2.5 px-8 py-3 bg-[#5600e3] hover:bg-[#4500b6] text-white rounded-full text-[15px] font-bold transition-all shadow-sm">
                    ...
                    <div className="rounded-3xl bg-[rgba(236,237,241,0.85)] backdrop-blur-[12px] shadow-[inset_0_4px_8px_rgba(0,0,0,0.08),inset_0_-2px_4px_rgba(255,255,255,0.9),0_2px_8px_rgba(0,0,0,0.06)] px-6 py-5 flex flex-col gap-1">
                        {navItems.map(({ labelKey, id, href }) => (
                            <button
                                ...
                                className="text-left text-[15px] font-bold text-slate-700 hover:text-slate-900 transition-colors bg-transparent border-none cursor-pointer py-3 border-b border-slate-200/60 last:border-b-0"
                            >
                                {t(labelKey)}
                            </button>
                        ))}
                        <LanguageSwitcher mobile />
                        <button onClick={() => setModalOpen(true)} className="mt-3 flex items-center justify-center gap-2.5 px-8 py-3.5 bg-[#5600e3] hover:bg-[#4500b6] text-white rounded-full text-[15px] font-bold transition-all shadow-sm w-full">
                            <span className="relative flex items-center justify-center w-2 h-2">
                                <span className="absolute inline-flex h-full w-full rounded-full bg-green-300 opacity-80 animate-ping" />
                                <span className="relative inline-flex rounded-full h-2 w-2 bg-green-400" />
                            </span>
